...
export default {
  ...
  methods: {
    ...
    upload () {
      ...
      fileDatas.append(this.name, this.file)
      if (this.isMinImg) {
        fileDatas.append(`${this.name}_min`, this.minImgFile)
      }
      for (const key in this.data) {
        fileDatas.append(key, this.data[key])
      }

      this.$emit('on-cropper-end')
